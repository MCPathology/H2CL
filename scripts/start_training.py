...
from better_mistakes.model.run_xent import run, run_GD
...
def load_data(train_csv, val_csv, distributed):
    ...
    train_albu_transform = albu.Compose([
        ...
        albu.Resize(opts.target_size, opts.target_size, interpolation=cv2.INTER_LINEAR, always_apply=True),
        ])
    ...
    st = time.time()
    dataset = InputDataset(train_csv, True, train_transform, opts.data,
            albu_transform=train_albu_transform)
    print("Took", time.time() - st)

    print("Loading validation data")
    dataset_test = InputDataset(val_csv, False, test_transform, opts.data,
    ...
    for epoch in range(opts.start_epoch, opts.epochs):
        ...
        if opts.visualization:
            summary_train, steps = run_nn(
                train_loader, model, loss_function, opts, epoch, steps, emb_layer, embeddings_mat, optimizer, is_inference=False,
            )
        else:
            if opts.data == 'GD':
...
if __name__ == "__main__":
    ...
    # Data/paths ----------------------------------------------------------------------------------------------------------------------------------------------
    parser.add_argument("--data", default="cervix", help="id of the dataset to use: | ".join(DATASET_NAMES))
